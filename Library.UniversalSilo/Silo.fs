...
module Configuration =
    ...
    type public SiloConfigurator() = class
        ...
        default __.ConfigureDashboard configuration siloSettings siloBuilder =            
            logger.LogInformation(
                "Cannot start dashboard for clustering mode {ClusteringMode} until Dashboard moves to Orleans 4",
                siloSettings.ClusteringConfiguration.ClusteringMode)
            raise <| NotSupportedException("Dashboard not yet supported - it is still using Orleans 3.x")
