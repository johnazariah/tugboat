...
push-library : pack-library
	$(MAKE) package_name=Tugboat package_version=$(LibraryVersion) push

pack-library :
	$(MAKE) project_path=Library.Tugboat/Library.Tugboat.csproj package_name=Tugboat package_version=$(LibraryVersion) pack
...
push-template-pack : clean-template-pack copy-template-pack pack-template-pack
	$(MAKE) package_name=Tugboat.Templates package_version=$(LibraryVersion) push
...
copy-single-template.webapi-directclient : copy-single-template.% : copy-common.% copy-appl-controllers.% copy-project.%
	@echo Built Template Folder For $* [$(lang)]
	- rm $(copy_target_root)/$*-$(lang)/GeneratedProjectName/$*.$(projsuffix)
	@echo
...
copy-common.% : copy-grains.% copy-appl-tests.% copy-templates.% copy-ignores.%
...
replace-project-reference-with-nuget-reference :
	- sed -e "s/<ProjectReference.*Library.Tugboat.csproj\"/<PackageReference Include=\"Tugboat\" Version=\"$(LibraryVersion)\"/g" $(proto_root)-$(lang)/$(src_project_file) > $(copy_target_root)/$(dest_project_file)

pack-template-pack :
	$(MAKE) project_path=$(scratch)/build/Tugboat.Templates.csproj package_name=Tugboat.Templates package_version=$(LibraryVersion) pack

uninstall-template-pack :
	- dotnet new -u Tugboat.Templates

install-template-pack : uninstall-template-pack
	dotnet new -i Tugboat.Templates.$(LibraryVersion).nupkg
